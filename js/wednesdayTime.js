...
$("#saveProgram").click(function () {
    var num = getProgram(Days.Wednesday).length;
    for (var i = 1; i <= num; i++) {
        removePeriod(Days.Wednesday, 0);
    }

    for (var i = 1; i <= 5; i++) {
        if ($("#row" + i).is(":visible")) {
            addPeriod(Days.Wednesday, $("#day" + i).val(), $("#night" + i).val());
        }
    }
...
function fillWeek() {
    getWeekProgram();
    var times = getProgram(Days.Wednesday)
